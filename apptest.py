...
def compute_object_area_categories(water_map, stats):
    ...
    Returns:
        summary_dict, labeled_map, label_categories
    """
    wm = np.array(water_map)
    water_mask = get_water_mask(wm)
    ...
    for label_idx, count in zip(indices, pixel_counts):
        ...
        label_categories[label_idx] = key

    return result, labeled, label_categories
...
def make_category_color_mask(labeled, label_categories):
    ...
    # Colors (RGB)
    color_map = {
        "very_small": (249, 212, 4),  # light gray
...
if uploaded_past is not None and uploaded_present is not None:
    try:
        ...
        if (
            ...
            cat_past, labels_past, label_cat_past = compute_object_area_categories(water_past, stats_past)
            cat_present, labels_present, label_cat_present = compute_object_area_categories(water_present, stats_present)
            ...
            st.markdown("### 🗺️ Water Bodies by Size Category")
            render_size_legend()
            col_cat_past, col_cat_present = st.columns(2)
            with col_cat_past:
                if category_mask_past is not None:
                    st.image(
                        category_mask_past,
                        use_container_width=True,
                        caption="Past – Water Bodies by Size Category",
                    )
                else:
                    st.caption("No water bodies detected in the past image.")
            with col_cat_present:
                if category_mask_present is not None:
                    st.image(
                        category_mask_present,
                        use_container_width=True,
                        caption="Present – Water Bodies by Size Category",
                    )
                else:
