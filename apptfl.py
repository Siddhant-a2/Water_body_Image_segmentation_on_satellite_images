...
st.set_page_config(page_title="Water Mapping (Past vs Present) and Area Calculation and Comparison", layout="wide")
...
st.markdown(
    '<div class="center-title">Surface Water Mapping (Past vs Present) and Area Calculation and Comparison</div>',
    unsafe_allow_html=True,
)
st.markdown(
...
def read_tif_to_array(uploaded_file):
    ...
    - img: (H, W, C) float32 array, normalized to ~0–1 reflectance
    ...
    # ---------- NEW: auto-normalize Sentinel-2 data ----------
    # Compute a robust max to avoid outliers
    vmax = float(np.percentile(img, 99.9))

    # Case 1: values look like DN (0–10000) → scale down
    if vmax > 10.0:
        # Typical Sentinel-2 L2A scaling
        img = img / 10000.0

    # Case 2: values already look like reflectance (0–1 or 0–2) → leave as is
    # (no extra scaling needed)

    # Clip to [0,1] to be safe for the model
